...
func AddFileLocation(id int64, path string, name string, length float64) error {
	...
	log.Printf("[AddFileLocation] Створено або оновлено запис: ID %d для Name %s, Path: %s розміром %f", id, name, path, length)
	return nil
}

func GetFileInfo(id int64) (string, string, float64, error) {
	query := `SELECT path, name, length FROM files_info WHERE id = ?;`
